...
dispatch_kernel(void* packed_recv_x, float* packed_recv_x_scales,
    ...
    if (sm_id > 0 && sm_id <= kNumRdmaRanks) {
        ...
        // init atomic_counter_per_expert
        if (sm_id == 1) {
        ...
        if (thread_id == 0) {
            ...
            // printf("src_rank: %d send msg to dst_rank: %d, num_tokens_sent: %d\n", rank, dst_rdma_rank * NUM_MAX_NVL_PEERS + nvl_rank, num_tokens_sent);
            nvshmemi_ibgda_amo_nonfetch_add(
                reinterpret_cast<int*>(dst_ptr), // rptr
                -num_tokens_sent - 1, // value
                dst_rdma_rank * NUM_MAX_NVL_PEERS + nvl_rank,  // dst_pe
                dst_rdma_rank,  // qp_id
                dst_rdma_rank == rdma_rank); // local_copy?
    ...
    // if (sm_id < kNumRdmaRanks) {
    {
        ...
        for (int rdma_recv_token_idx = sub_rdma_rank; rdma_recv_token_idx < num_recv_tokens_per_rdma; rdma_recv_token_idx += sms_per_rdma) {
            ...
            for (int loop_nvl_expert_i = warp_id; loop_nvl_expert_i < dst_nvl_experts; loop_nvl_expert_i += num_warps) {
                ...
                UNROLLED_WARP_COPY(UNROLL_FACTOR, lane_id, hidden_int4, dst_data + 1, src_data, ld_nc_global, st_na_global);
        ...
        if (sub_rdma_rank == 0 && thread_id == 0) {
            while (ld_acquire_global(atomic_nvl_sender_multi_sms + src_rdma_rank) != num_recv_tokens_per_rdma);
        }
        __syncthreads();
        // 设置对端标志位
        if (sub_rdma_rank == 0) {
...
combine_kernel(void* combined_x, // 结果 num_combined_tokens * kHidden
    ...
    // 根据dispatch的rdma_recv_x中的内容，去不同nvl buffer中拿结果进行reduce，并放到rdma send buffer中
    {
        ...
        const int num_tokens_to_deal = dispatch_rdma_recv_count[deal_rdma_rank];
        ...
        // all sms reduce done
        if (thread_id == 0) {
            while (ld_acquire_global(atomic_nvl_sender_multi_sms + deal_rdma_rank) != num_tokens_to_deal);
        }
        __syncthreads();
        // set flag
        if (sub_deal_rdma_rank == 0 && warp_id == 0) {
            // notify remote rdma
            if (thread_id == 0) {
                ...
                nvshmemi_ibgda_amo_nonfetch_add(
                    ...
                    deal_rdma_rank * NUM_MAX_NVL_PEERS + nvl_rank,  // dst_pe
                    deal_rdma_rank,  // qp_id
                    deal_rdma_rank == rdma_rank); // local_copy?
            }
        }
        ...
        for (int token_idx = sm_id; token_idx < num_combined_tokens; token_idx += num_sms) {
            ...
            for (int rdma_rank_idx = 0; rdma_rank_idx < kNumRdmaRanks; ++rdma_rank_idx) {
                if (rdma_send_flags_now[rdma_rank_idx]) {
                    // 说明传到了对应rdma上，累加结果
                    const int4* src_ptr = reinterpret_cast<int4*>(reinterpret_cast<uint8_t*>(rdma_recv_x) + (rdma_rank_idx * num_max_dispatch_tokens_per_rank + token_idx) * combine_hidden_bytes);
                    auto x_vec = ld_nc_global(src_ptr + thread_id);
                    const auto x_bf16 = reinterpret_cast<nv_bfloat16*>(&x_vec);
                    #pragma unroll
                    for (int j = 0; j < kNumElemsPerInt4; ++ j)
