...
class Buffer:
    ...
    def __init__(self, group: dist.ProcessGroup,
        ...
        # print("low_latency_mode: ", low_latency_mode)
        ...
        if self.runtime.get_num_rdma_ranks() > 1 or low_latency_mode:
            ...
            os.environ['NVSHMEM_IBGDA_NUM_RC_PER_PE'] = f'{64}'
    ...
    def get_low_latency_rdma_size_hint_two_stage(num_max_dispatch_tokens_per_rank: int, hidden: int, num_ranks: int, num_experts: int, num_topk: int) -> int:
        """
        Get a minimum size requirement for the RDMA buffer. The size calculation will be done with BF16.
        ...
        Returns:
            size: the RDMA buffer size recommended.
        """
        return deep_ep_cpp.get_low_latency_rdma_size_hint_two_stage(num_max_dispatch_tokens_per_rank, hidden, num_ranks, num_experts, num_topk)
    
    def get_low_latency_nvl_size_hint_two_stage(num_max_dispatch_tokens_per_rank: int, hidden: int, num_ranks: int, num_experts: int, num_topk: int, use_fp8: bool) -> int:
        """
        Get a minimum size requirement for the RDMA buffer. The size calculation will be done with BF16.
        ...
        Returns:
            size: the RDMA buffer size recommended.
        """
        return deep_ep_cpp.get_low_latency_nvl_size_hint_two_stage(num_max_dispatch_tokens_per_rank, hidden, num_ranks, num_experts, num_topk, use_fp8)
    ...
    def low_latency_dispatch_two_stage(self, x: torch.Tensor, topk_idx: torch.Tensor, topk_weights: torch.Tensor,
                                        num_max_dispatch_tokens_per_rank: int, num_experts: int,
                                        use_fp8: bool = True, async_finish: bool = False, return_recv_hook: bool = False) -> \
            Tuple[Tuple[torch.Tensor, torch.Tensor], torch.Tensor, Tuple, EventOverlap, Callable]:
        """
    ...
    def low_latency_combine_two_stage(self, x: torch.Tensor, topk_idx: torch.Tensor, topk_weights: torch.Tensor,
                                    handle: tuple, dispatch_use_fp8: bool = False, async_finish: bool = False,
                                    return_recv_hook: bool = False, out: Optional[torch.Tensor] = None) -> \
            Tuple[torch.Tensor, EventOverlap, Callable]:
        """
        A low-latency implementation for combining tokens (reduce **with weights**) with IBGDA.
