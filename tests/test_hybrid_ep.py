...
def bitwise_equal(a: torch.Tensor, b: torch.Tensor) -> bool:
    ...
    # Use (a == b).all() pattern which works better with Paddle compat layer
    equal_result = (a_bytes == b_bytes)
    all_equal = equal_result.all()
    # Extract Python bool from the result
    if hasattr(all_equal, 'numpy'):
        return bool(all_equal.numpy())
...
def test_hybrid_ep_correctness(buffer: deep_ep.HybridEPBuffer, ref: TorchRef, use_fp8: bool):
    ...
    for with_probs in [True]:
        ...
        copy_times = local_expert_routing_map.sum(dim=1).to(torch.bfloat16) 
        ...
        assert torch.allclose(combined_hidden.to(torch.float32), hidden.to(torch.float32), atol=2e-5, rtol=1e-2)
